...
buffer = await getBuffer('https://raw.githubusercontent.com/alonecans/foto-manca/main/me%20(1).jpg')
	...
	case 'asupanghea':
			 ...
			 fxsx.sendMessage(from, buffer, video, {quoted: {key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","height":736,"width":736,"jpegThumbnail":fs.readFileSync('./stik/thumb.jpeg)},"productId":"3937202479680283","title":"SEKEDAR UNTUK GABUT","currencyCode":"IDR","priceAmount1000":"9999","productImageCount":1},"businessOwnerJid":"0@s.whatsapp.net"}}}})
			 break
    case 'quotes':
			 if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
			 if (!isBanned) return fxsx.sendMessage(from, `Maaf ${pushname} Kamu Sudah Terbanned`,MessageType.text, { quoted: ftroli})
	...
	case 'self':
	        if (!isOwner) return fxsx.sendMessage(from, 'Khusus Owner Pinky',MessageType.text, { quoted: ftroli})
         	...
         	banChats = true
          	...
          	break
    case 'banlist':
            if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})if (!isUser) return fxsx.sendMessage(from, `Hai ${pushname}\nKamu Belum Terdaftar Silahkan\nKetik : ${prefix}daftar`,MessageType.text, {quoted: ftroli})
			if (!isBanned) return fxsx.sendMessage(from, `Maaf ${pushname} Kamu Sudah Terbanned`,MessageType.text, { quoted: ftroli})
			ben = '```List Banned``` :\n'
...
${descOwner ? `*Desc diubah oleh* : @${descOwner.split('@')[0]}` : '*Desc diubah oleh* : -'}\n*Tanggal* : ${descTime ? `${formatDate(descTime * 1000)}` : '-'}\n\n*Kontak yang tersimpan*\n`
    ...
    case 'daftar':
			 ...
			 fxsx.sendMessage(from, buffer, image, {quoted: {"key": {   "fromMe": false,"participant":"0@s.whatsapp.net",   "remoteJid": "0@s.whatsapp.net"  }, "message": {orderMessage: {itemCount: 1,status: 200, thumbnail: fs.readFileSync('./stik/thumb.jpeg'), surface: 200, message: `${fake}\nHai ${pushname}`, orderTitle: 'NuyFaa', sellerJid: '0@s.whatsapp.net'}}, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: captionnya}})
			 break
default:
if (!isOwner) return fxsx.sendMessage(from, 'Khusus Owner Pinky',MessageType.text, { quoted: ftroli})
if (budy.startsWith('>')){
